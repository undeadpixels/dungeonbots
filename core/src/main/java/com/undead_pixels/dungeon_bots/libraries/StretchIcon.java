 ...
 */
package com.undead_pixels.dungeon_bots.libraries;
 ...
 * @author Darryl, support for predefined width/height by WO
 */
public class StretchIcon extends ImageIcon {
	...
	public StretchIcon(byte[] imageData, int width, int height) {
		this(imageData);
	...
	public StretchIcon(Image image, int targetWidth, int targetHeight){
		this(image);
	...
	public synchronized void paintIcon(Component c, Graphics g, int x, int y) {
		...
		int w = targetWidth >= 0 ? targetWidth : c.getWidth() - x - insets.right;
		int h = targetHeight >= 0 ? targetHeight : c.getHeight() - y - insets.bottom;
