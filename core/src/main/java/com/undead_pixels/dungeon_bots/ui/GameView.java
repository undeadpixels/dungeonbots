...
public class GameView extends GDXandSwingScreen implements InputProcessor {
	private Stage stage = new Stage(); // deleting this somehow makes it not
										// work...?
	...
	public GameView() {
		...
		// Player.PLAYER_TEXTURE = new TextureRegion(new
		// Texture("DawnLike/Characters/Player0.png"), TILESIZE *1, TILESIZE *1,
		// TILESIZE, TILESIZE);
		world = new World();
		...
		Vector2[] offsetsWalls = new Vector2[] { new Vector2(1, 1), // 0 default
		...
		Vector2[] offsetsFloors = new Vector2[] { new Vector2(5, 0), // 0
																		// default
		...
		LuaScript luaScript = sandbox.script(new File("sample-level-packs/sample-pack-1/levels/level1.lua")).start()
				.join();
	...
	public void render(float dt) {
		...
		if (!didInitCam) {
			...
			// cam = new
			// AFixedOrthographicCameraBecauseGDXsDefaultOrthographicCameraHasABugInItThatTookHoursToFigureOut(w,
			// h);
	...
	public boolean touchDown(int screenX, int screenY, int pointer, int button) {
		...
		System.out.println("Clicked entity " + e + " at " + gameSpace.x + ", " + gameSpace.y + " (screen " + screenX
				+ ", " + screenY + ")");
